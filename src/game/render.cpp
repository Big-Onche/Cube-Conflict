    ...
    void renderplayer(gameent *d, const playermodelinfo &mdl, int color, int team, float fade, int flags = 0, bool mainpass = true)
    {
        ...
        else if(!intermission && forcecampos<0)
        {
            ...
            else
            {
                ...
                if(d->move && d->physstate==PHYS_FLOOR && rndevent(95)) particle_splash(atmos && (lookupmaterial(d->feetpos())==MAT_WATER || isRaining) ? PART_WATER : PART_SMOKE, powerArmor ? 5 : 3, 120, d->feetpos(), isRaining && atmos ? 0x131313 : mapatmosphere==9 ? 0xFFFFFF : 0x333022, 6.0f+rnd(d->armourtype==A_POWERARMOR ? 10 : 5), 150, 15);
            }

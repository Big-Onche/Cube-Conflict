...
enum { RAY_BB = 1, RAY_POLY = 3, RAY_ALPHAPOLY = 7, RAY_ENTS = 9, RAY_CLIPMAT = 16, RAY_SKIPFIRST = 32, RAY_EDITMAT = 64, RAY_PASS = 128, RAY_LIQUIDMAT = 256};
...
enum
{
    ...
    PART_FLAME,
    PART_STREAK,
    PART_RAIL_TRAIL, PART_PULSE_SIDE, PART_PULSE_FRONT, PART_BALLE_SIDE, PART_POMPE_SIDE, PART_IMPACT, PART_SPOCK_FRONT,
    PART_SANTE, PART_MANA,
    PART_FLAME1, PART_FLAME2, PART_EAU, PART_BULLE,
    PART_SNOW, PART_RAIN, PART_NUAGE1, PART_NUAGE2, PART_NUAGE3, PART_NUAGE4, PART_RAINBOW,
    PART_VISEUR, PART_ZERO, PART_ONE, PART_BLIP,
    PART_SPARK,
    ...
    PART_LIGHTNING,
    PART_EXPLOSION, PART_PLASMABURST, PART_ONDECHOC, PART_PLASMAGRENADE, PART_RADAR,
    PART_TEXT,
...
extern void particle_splash(int type, int num, int fade, const vec &p, int color = 0xFFFFFF, float size = 1.0f, int radius = 150, int gravity = 2, int sizemod = 0, bool randomcolor = false, float relativesize = 1.f);
...
// rendermodel
enum { MDL_CULL_VFC = 1<<0, MDL_CULL_DIST = 1<<1, MDL_CULL_OCCLUDED = 1<<2, MDL_CULL_QUERY = 1<<3, MDL_FULLBRIGHT = 1<<4, MDL_NORENDER = 1<<5, MDL_MAPMODEL = 1<<6, MDL_NOBATCH = 1<<7, MDL_ONLYSHADOW = 1<<8, MDL_NOSHADOW = 1<<9, MDL_FORCESHADOW = 1<<10, MDL_FORCETRANSPARENT = 1<<11, MDL_CULL_EXTDIST = 1<<12};
...
enum { DISC_NONE = 0, DISC_EOP, DISC_LOCAL, DISC_KICK, DISC_MSGERR, DISC_DESYNC, DISC_UNK, DISC_MSGERR_DEF, DISC_MSGERR_MINUS1, DISC_MSGERR_SERVMSG, DISC_IPBAN, DISC_PRIVATE, DISC_MAXCLIENTS, DISC_TIMEOUT, DISC_OVERFLOW, DISC_PASSWORD, DISC_NUM, DISC_NORMAL };

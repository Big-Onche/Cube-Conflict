...
namespace buttons
{
    struct Button
    {
        ...
        // Constructor
        Button(SDL_Renderer* renderer, int x, int y, int width, int height, int color, int hoverColor, const std::string& text, int textureId, std::function<void()> action)
                : x(x), y(y), width(width), height(height), color(color), hoverColor(hoverColor), text(text), textureId(textureId), action(std::move(action)) {
        }

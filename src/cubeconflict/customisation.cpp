...
void buyitem(int itemtype) //Achète un objet
{
    switch(itemtype)
    {
        case 0:
            ...
            else if(stat[SMI_HAP+UI_smiley]>0) {conoutf(CON_GAMEINFO, "\f3Vous possédez déjà ce smiley !"); playsound(S_ERROR); return;}
            else
            {
                conoutf(CON_GAMEINFO, "\f9Smiley \"%s\" acheté !", customssmileys[UI_smiley].smileyname);
        ...
        case 1:
            if(customscapes[UI_cape].capeprice > stat[STAT_CC]) {conoutf(CON_GAMEINFO, "\f3Cette cape est trop chère pour toi !"); playsound(S_ERROR); return;}
            else if(stat[CAPE_CUBE+UI_cape]>0) {conoutf(CON_GAMEINFO, "\f3Vous possédez déjà cette cape !"); playsound(S_ERROR); return;}
            else
            {
                conoutf(CON_GAMEINFO, "\f9Cape \"%s\" achetée !", customscapes[UI_cape].capename);
        ...
        case 2:
            if(customstombes[UI_tombe].tombeprice > stat[STAT_CC]) {conoutf(CON_GAMEINFO, "\f3Cette tombe est trop chère pour toi !"); playsound(S_ERROR); return;}
            else if(stat[TOM_MERDE+UI_tombe]>0) {conoutf(CON_GAMEINFO, "\f3Vous possédez déjà cette tombe !"); playsound(S_ERROR); return;}
            else
            {
                conoutf(CON_GAMEINFO, "\f9Tombe \"%s\" achetée !", customstombes[UI_tombe].tombename);
        ...
        case 3:
            ...
            else if(stat[VOI_CORTEX+UI_voix]>0) {conoutf(CON_GAMEINFO, "\f3Vous possédez déjà cette voix !"); playsound(S_ERROR); return;}
            else
            {

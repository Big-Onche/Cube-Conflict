...
void buyitem(int itemtype) //Achète un objet
{
    switch(itemtype)
    {
        case 0:
            ...
            else if(stat[SMI_HAP+UI_smiley]>0) {conoutf(CON_GAMEINFO, langage ? "\f3You already own this smiley!" : "\f3Vous possédez déjà ce smiley !"); playsound(S_ERROR); return;}
            else
            {
        ...
        case 1:
            ...
            else if(stat[CAPE_CUBE+UI_cape]>0) {conoutf(CON_GAMEINFO, langage ? "\f3You already own this cape!" : "\f3Vous possédez déjà cette cape !"); playsound(S_ERROR); return;}
            else
            {
        ...
        case 2:
            ...
            else if(stat[TOM_MERDE+UI_tombe]>0) {conoutf(CON_GAMEINFO, langage ? "\f3You already own this grave!" : "\f3Vous possédez déjà cette tombe !"); playsound(S_ERROR); return;}
            else
            {

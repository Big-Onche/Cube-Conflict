...
namespace game
{
    ...
    {
        grass::addImpulse(v, vel, 22.0f, 0.9f, 450, grass::IMPULSE_BULLET, 0, 0.35f, 0.4f);
    ...
    void renderBulletImpact(gameent *owner, const vec &v, const vec &vel, dynent *safe, int atk) //particles and light effects on impact for fast projectiles
    {
        ...
        grass::addImpulse(v, vel, heavy ? 24.0f : atk == ATK_CROSSBOW ? 12.0f : 18.0f, heavy ? 1.0f : 0.8f, heavy ? 600 : 450, grass::IMPULSE_BULLET, 0, 0.35f, 0.4f);
    ...
    void renderInstantImpact(const vec &from, const vec &to, const vec &muzzle, int atk, bool hasRoids) //particles and light effects on impact for instant projectiles
    {
        ...
        if(atk != ATK_FLAMETHROWER)
            grass::addImpulse(to, vec(to).sub(from), heavy ? 28.0f : 22.0f, heavy ? 1.0f : 0.9f, heavy ? 400 : 350, grass::IMPULSE_BULLET, 0, 0.35f, 0.4f);

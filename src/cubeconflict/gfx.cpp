...
namespace game
{
    ...
    {
        addgrassimpulse(v, vel, 22.0f, 0.9f, 450, GRASS_IMPULSE_BULLET, 0, 0.35f, 0.4f);
        ...
        switch(atk)
        {
            case ATK_PLASMA:
            {
                if(!inWater(lightOrigin)) particle_splash(PART_FIRESPARK, 15, 150, v, hasRoids ? 0xFF0000 : 0xFF6600, 1.f, 150, 500, 2, hasShrooms());
            ...
            case ATK_GRAP1:
            {
                if(!inWater(lightOrigin)) particle_splash(PART_SPARK, 20, 100, v, hasRoids ? 0xFF0000 : 0xAA4466, 0.5f, 400, 400, 0, hasShrooms());
            ...
            case ATK_SPOCKGUN:
            {
                if(!inWater(lightOrigin)) particle_splash(PART_SPARK, 15, 100, v, hasRoids ? 0xFF0000 : 0x00FF66, 0.5f, 250, 250, 0, hasShrooms());
                particle_splash(PART_HAZE_BIG, 4, 300, v, 100, 7.f, 50, 100, 40);
    ...
    void renderProjectilesTrails(gameent *owner, vec &pos, vec dv, vec &from, vec &offset, int atk, bool exploded) //particles and light effects on impact for slow projectiles
    {
        ...
        switch(atk)
        {
            case ATK_PLASMA:
                ...
                if(isInWater && emitPart) particle_splash(PART_BUBBLE, 1, 150, pos, 0x18181A, 2.0f+rnd(2), 20, -30);
            ...
            case ATK_FAMAS:
            {
                if(isInWater && emitPart) particle_splash(PART_BUBBLE, 1, 150, pos, 0x18181A, 1.0f+rnd(2), 20, -30);
            ...
            case ATK_CROSSBOW:
                if(!exploded)
                {
                    if(emitPart)
                    {
                        if(isInWater) particle_splash(PART_BUBBLE, 1, 150, pos, 0x888888, 1.0f+rnd(2), 20, -30);
            ...
            case ATK_FIREWORKS:
                if(emitPart)
                {
                    if(isInWater) particle_splash(PART_BUBBLE, 3, 200, pos, 0x18181A, 2.5f, 25, 100, 0, hasShrooms());
        ...
        switch(atk)
        {
            ...
            case ATK_S_ROCKETS:
            {
                ...
                loopi(isInWater ? 3 : 8)
                {
                    ...
                    if(!isInWater) particle_splash(PART_SPARK, miniRockets ? 1 : 2, 300, pos, hasRoids ? 0xFF4444 : 0xFFBB55,  1.2f+rnd(2), 2500, 2500, -1, hasShrooms());
                ...
                if(!epilepsyfriendly)
                {
                    adddynlight(v, 4*attacks[atk].exprad, vec(5.0f, 2.2f, 0.0f), 80, 40, flags, initRadius, vec(1.f, 0.f, 0.f));
                    adddynlight(v, 2*attacks[atk].exprad, vec(1.5f, 0.5f, 0.0f), 80, 40, L_VOLUMETRIC|flags, initRadius, vec(1.0f, 0.f, 0.f));
                }
                return;
            }
            case ATK_FIREWORKS:
                ...
                if(!epilepsyfriendly) adddynlight(v, 3*attacks[atk].exprad, vec(0.7f, 0.7f, 0.7f), 80, 40, L_VOLUMETRIC|flags, initRadius, vec(0.5f, 0.5f, 0.3f));
                return;
            ...
            case ATK_M32:
                ...
                if(!epilepsyfriendly)
                {
                    adddynlight(v, 4*attacks[atk].exprad, vec(0.0f, 2.5f, 7.0f), 80, 40, flags, initRadius, vec(0.5f, 1.5f, 2.0f));
                    adddynlight(v, 3*attacks[atk].exprad, vec(0.0f, 0.5f, 1.5f), 80, 40, L_VOLUMETRIC|flags, initRadius, vec(0.0f, 0.0f, 1.5f));
                }
                return;
            ...
            case ATK_MOLOTOV:
            {
                ...
                {
                    vec pos = v;
                    if(!isInWater) pos.add(vec(-30+rnd(61), -30+rnd(61), -10+rnd(21)));
                ...
                if(!epilepsyfriendly) adddynlight(v, 2.5*attacks[atk].exprad, vec(2.0f, 0.7f, 0.0f), 250, 40, L_VOLUMETRIC|flags, initRadius, vec(1.5f, 0.0f, 0.0f));
                return;
            }
            case ATK_KAMIKAZE:
            case ATK_POWERARMOR:
            {
                ...
                if(!epilepsyfriendly)
                {
                    adddynlight(v, 5*attacks[atk].exprad, vec(5.0f, 3.0f, 0.0f), 80, 40, flags, initRadius, vec(1.f, 0.f, 0.f));
                    adddynlight(v, 3*attacks[atk].exprad, vec(1.5f, 0.5f, 0.0f), 80, 40, L_VOLUMETRIC|flags, initRadius, vec(1.f, 0.f, 0.f));
                }
                return;
            }
            case ATK_S_NUKE:
                ...
                if(!epilepsyfriendly)
                {
                    ...
                    adddynlight(v, 3*attacks[atk].exprad, vec(8.0f, 4.0f, 0.0f), 300, 40, L_NOSHADOW, initRadius, vec(0.5f, 1.5f, 2.0f));
                    adddynlight(v, 2*attacks[atk].exprad, vec(8.0f, 4.0f, 0.0f), 300, 40, L_NOSHADOW|L_VOLUMETRIC, initRadius, vec(0.0f, 0.0f, 1.5f));
                }
                return;
        }
    }
    ...
    void renderBulletImpact(gameent *owner, const vec &v, const vec &vel, dynent *safe, int atk) //particles and light effects on impact for fast projectiles
    {
        ...
        addgrassimpulse(v, vel, heavy ? 24.0f : atk == ATK_CROSSBOW ? 12.0f : 18.0f, heavy ? 1.0f : 0.8f, heavy ? 600 : 450, GRASS_IMPULSE_BULLET, 0, 0.35f, 0.4f);
    ...
    void renderInstantImpact(const vec &from, const vec &to, const vec &muzzle, int atk, bool hasRoids) //particles and light effects on impact for instant projectiles
    {
        ...
        if(atk != ATK_FLAMETHROWER) addgrassimpulse(to, vec(to).sub(from), heavy ? 28.0f : 22.0f, heavy ? 1.0f : 0.9f, heavy ? 400 : 350, GRASS_IMPULSE_BULLET, 0, 0.35f, 0.4f);

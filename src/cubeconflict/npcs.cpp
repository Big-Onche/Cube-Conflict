                ...
                else
                {
                    ...
                    if(!npcs[m.mtype].friendly)
                    {
                        ...
                        else
                        {
                            ...
                            if(m.move && m.physstate==PHYS_FLOOR && rndevent(95)) particle_splash(atmos && (lookupmaterial(m.feetpos())==MAT_WATER || mapatmosphere==4 || mapatmosphere==8) ? PART_WATER : PART_SMOKE, 2, 120, m.feetpos(), atmos && mapatmosphere==4 ? 0x131313 : 0x333022, 6.0f+rnd(5), 150, 15);
                        }
                    }

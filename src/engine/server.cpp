...
#ifdef WIN32
    MessageBox(NULL, msg, "Cube Conflict fatal error", MB_OK|MB_SYSTEMMODAL);
#else
...
const char *disconnectreason(int reason)
{
    ...
    switch(reason)
    {
        ...
        case DISC_MSGERR:           return lang ? "network message error (unspecified)" : "erreur message réseau (non spécifié)";
...
static void setupwindow(const char *title)
{
	...
	if(!appwindow) fatal("Impossible de lancer le programme (erreur fenêtre)");

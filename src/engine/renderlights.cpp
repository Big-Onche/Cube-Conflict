...
VARFP(msaa, 0, 0, 16, initwarning(readstr("Setting_MsaaSetup"), INIT_LOAD, CHANGE_SHADERS));
VARF(msaadepthstencil, 0, 2, 2, initwarning(readstr("Setting_MsaaSetup"), INIT_LOAD, CHANGE_SHADERS));
VARF(msaastencil, 0, 0, 1, initwarning(readstr("Setting_MsaaSetup"), INIT_LOAD, CHANGE_SHADERS));
VARF(msaaedgedetect, 0, 1, 1, cleanupgbuffer());
VARFP(msaalineardepth, -1, -1, 3, initwarning(readstr("Setting_MsaaSetup"), INIT_LOAD, CHANGE_SHADERS));
...
VAR(msaalight, 1, 0, 0);
VARF(msaapreserve, -1, 0, 1, initwarning(readstr("Setting_MsaaSetup"), INIT_LOAD, CHANGE_SHADERS));
...
void setupgbuffer()
{
    ...
    bool scaled = sw != hudw || sh != hudh;
    if(gw == sw && gh == sh && (scaled ? scalefbo[0] && scalew == hudw && scaleh == hudh : !scalefbo[0])) return;
    ...
    if(scaled) setupscale(gw, gh, hudw, hudh);
}
...
FVARP(bloomscale, 0, 1.3f, 1e3f);
VARP(bloomblur, 0, 7, 7);
VARP(bloomiter, 0, 2, 4);
VARFP(bloomsize, 6, 8, 11, cleanupbloom());
VARFP(bloomprec, 0, 2, 3, cleanupbloom());
FVAR(hdraccumscale, 0, 0.9f, 1);
VAR(hdraccummillis, 1, 30, 1000);
VAR(hdrreduce, 0, 2, 2);
...
void processhdr(GLuint outfbo, int aa)
{
    ...
    GLOBALPARAMF(hdrparams, hdrbright*1.25f, hdrsaturate, bloomthreshold, bloomscale);

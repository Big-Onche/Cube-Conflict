...
void disconnect(bool async, bool cleanup, bool volontaire)
{
    ...
    if(curpeer)
    {
        ...
        clearEntsPos();
        mainmenu = 1;
        if(gameStat[STAT_DAMMAGERECORD] < game::player1->stats.damage/10) updateStat(game::player1->stats.damage/10, STAT_DAMMAGERECORD, true);
        if(game::player1->stats.damage/10 > 10000) unlockAchievement(ACH_DESTRUCTEUR);
    }
